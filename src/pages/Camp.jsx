import SummercampForm from "../components/summercampForm";

const Camp = () => {
    return (
        <>
            <h4>School Age Camp!</h4>

            <p>Our Camp offers hands-on experiences in both the visual arts and culinary arts, perfect for young artists and aspiring chefs alike.</p>

            <h5>Camp Dates:</h5>
            <p>Winter Break, March Break, PA Days, Summer Vacation</p>
            <p>Monday to Friday | 9:00 AM - 5:00 PM</p>

            <h5>Arts Classes</h5>
            <p>Campers explore a wide range of mediums and techniques:</p>
            <ul>
                <li>Painting & Drawing</li>
                <li>Printmaking</li>
                <li>Mixed Media & Collage</li>
                <li>End of term art gallery</li>
            </ul>

            <h5>Cooking Classes</h5>
            <p>Each week features fun, age-appropriate kitchen adventures:</p>
            <ul>
                <li>Hands-on cooking and baking</li>
                <li>International cuisines</li>
                <li>Kitchen safety & teamwork</li>
            </ul>

            <h5>Ages:</h5>
            <p>Open to children ages 4+, grouped by age for activities</p>

            <h5>Cost: $35/Day</h5>
            <p>
                Limited Spots Available - Small Class Size = More 1 on 1 support<br/>
                Sibling discounts available<br/>
                All materials included
            </p>
            <SummercampForm/>
        </>
    );
}
  
export default Camp;